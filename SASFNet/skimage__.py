...
opt.use_cuda = True
opt.eps_value = 1e-4
opt.config_file = "./checkpoints/simple_batch_5/options.pkl"
opt.dataset_dir =  './datasets/training_data'
opt.checkpoints_file = './checkpoints/simple_batch_5/epoch_20_psnr_20.052.pth'
...
checkpointname = os.path.basename(opt.checkpoints_file)
modelname= checkpointname.split('.pt')[0]
opt.result = os.path.join('bi_lstm_results', modelname)
	...
	with torch.no_grad():
		...
		for cnt, (image_path, label_path) in enumerate(images_list, 1):
			try:
				...
				result_path = os.path.join(opt.result, imgname)
				image_pil = Image.fromarray(reflectance)
